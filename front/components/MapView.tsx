...
interface MapViewProps {
  ...
  snapshotTowers?: CellTowerMarker[];
  circles?: MapCircle[];
  drawingCircle?: { center: LatLng; radius: number; color: string } | null;
  phoneLocation?: { lat: number; lng: number; accuracy?: number | null } | null;
  computedLocation?: { lat: number; lng: number; radius?: number | null } | null;
...
const MapView: React.FC<MapViewProps> = ({ 
  ...
  return (
    ...
    >
      ...
      {/* Render Cell Towers (DB / within bbox) */}
      {towers.map((tower) => {
            ...
            <Popup>
              <div className="text-right text-xs space-y-1" dir="rtl">
                <p className="font-bold text-sm">
                  {tower.radio_type?.toUpperCase() ?? "UNKNOWN"} - CID:{" "}
                  {tower.cell_id ?? "نامشخص"}
                </p>
                <p>
      ...
      {/* Render Towers used for snapshot locate (anchors) */}
      {snapshotTowers.map((tower) => {
        // Use inline SVG (always visible) to avoid broken image/icon issues.
        const anchorIcon = L.divIcon({
          html: getIconHtml("circle", "#f59e0b"),
          className: "snapshot-anchor-icon",
          iconSize: [32, 32],
          iconAnchor: [16, 16],
          popupAnchor: [0, -16],
        });
        ...
        return (
          ...
          >
            <Popup>
              <div className="text-right text-xs space-y-1" dir="rtl">
                <p className="font-bold text-sm">دکل‌های استفاده‌شده در محاسبه</p>
                <p>
                  {tower.radio_type?.toUpperCase() ?? "UNKNOWN"} - CID:{" "}
                  {tower.cell_id ?? "نامشخص"}
                </p>
                ...
                <p className="font-mono">
                  {tower.lat.toFixed(7)}, {tower.lon.toFixed(7)}
                </p>
              </div>
            </Popup>
          </Marker>
      ...
      {/* Render Phone (Ground Truth) Location */}
      {phoneLocation && (
        <>
          <Marker
            key="phone-location"
            position={[phoneLocation.lat, phoneLocation.lng]}
            icon={L.divIcon({
              html: getIconHtml('pin', '#22c55e'),
              className: 'phone-location-icon',
              iconSize: [32, 32],
              iconAnchor: [16, 32],
              popupAnchor: [0, -32],
            })}
          >
            <Popup>
              <div className="text-right text-xs space-y-1" dir="rtl">
                <p className="font-bold text-sm">مکان گوشی (Ground Truth)</p>
                <p className="font-mono">
                  {phoneLocation.lat.toFixed(7)}, {phoneLocation.lng.toFixed(7)}
                </p>
                {phoneLocation.accuracy != null && <p>Accuracy: {Math.round(phoneLocation.accuracy)} m</p>}
              </div>
            </Popup>
          </Marker>
          {phoneLocation.accuracy != null && (
            <Circle
              center={[phoneLocation.lat, phoneLocation.lng]}
              radius={Number(phoneLocation.accuracy)}
      ...
      {/* Render Computed Location */}
      {computedLocation && (
        <>
          <Marker
            key="computed-location"
            position={[computedLocation.lat, computedLocation.lng]}
            icon={L.divIcon({
              html: getIconHtml('pin', '#a855f7'),
              className: 'computed-location-icon',
              iconSize: [32, 32],
              iconAnchor: [16, 32],
              popupAnchor: [0, -32],
            })}
          >
            <Popup>
              <div className="text-right text-xs space-y-1" dir="rtl">
                <p className="font-bold text-sm">مکان محاسبه‌شده</p>
                <p className="font-mono">
                  {computedLocation.lat.toFixed(7)}, {computedLocation.lng.toFixed(7)}
                </p>
                {computedLocation.radius != null && <p>Radius: {Math.round(computedLocation.radius)} m</p>}
              </div>
            </Popup>
          </Marker>
          {computedLocation.radius != null && (
            <Circle
              center={[computedLocation.lat, computedLocation.lng]}
              radius={Number(computedLocation.radius)}
              pathOptions={{ color: '#a855f7', fillColor: '#a855f7', fillOpacity: 0.10 }}
            />
          )}
        </>
      )}
              ...
              <Popup>
                <div className="text-right" dir="rtl">
                  ...
                  <p className="text-xs text-gray-500 font-mono mb-2">
                      {pin.position.lat.toFixed(7)}, {pin.position.lng.toFixed(7)}
                  </p>
                  <button 
