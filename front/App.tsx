...
import SnapshotLocate from './components/SnapshotLocate';
...
const App: React.FC = () => {
    ...
    const [snapshotResult, setSnapshotResult] = useState<SnapshotLocateResponse | null>(null);
    ...
    const handleSnapshotResult = (result: SnapshotLocateResponse) => {
        setSnapshotResult(result);

        const snapshotPins: Pin[] = [];
        if (result.phone_location) {
            snapshotPins.push({
                id: `snapshot:phone:${result.snapshot_id}`,
                name: `مکان گوشی (Snapshot ${result.snapshot_id})`,
                position: { lat: result.phone_location.lat, lng: result.phone_location.lng },
                color: '#22c55e',
                shape: 'pin',
                description: `Accuracy: ${result.phone_location.accuracy ?? '—'}m | type: ${result.phone_location.type ?? '—'}`,
            });
        }
        snapshotPins.push({
            id: `snapshot:computed:${result.snapshot_id}`,
            name: `مکان محاسبه‌شده (Snapshot ${result.snapshot_id})`,
            position: { lat: result.computed.location.lat, lng: result.computed.location.lon },
            color: '#a855f7',
            shape: 'pin',
            description: `Radius: ${Math.round(result.computed.radius)}m | source: ${result.computed.debug?.source ?? '—'}`,
        });

        setPins(prev => [
            ...prev.filter(p => !p.id.startsWith('snapshot:')),
            ...snapshotPins,
        ]);

        const target = result.phone_location
            ? { lat: result.phone_location.lat, lng: result.phone_location.lng }
            : { lat: result.computed.location.lat, lng: result.computed.location.lon };
        setMapCenter(target);
        setSelectedPinId(snapshotPins[0]?.id ?? null);
        setSidebarOpen(true);
    };
                            ...
                            <li key={pin.id} className="bg-white border rounded p-2 flex justify-between items-center hover:shadow-sm transition-shadow">
                                ...
                                >
                                    ...
                                    <div className="text-right">
                                        ...
                                        <div className="text-xs text-gray-400">{pin.position.lat.toFixed(7)}, {pin.position.lng.toFixed(7)}</div>
                                            </div>
                                        </div>
                                        <button 
            ...
            <div className="flex-1 relative">
                <div className="absolute top-4 left-4 z-[600] flex gap-2">
                    ...
                    <button
                        ...
                        className="bg-purple-500 text-white font-bold px-3 py-2 rounded-lg shadow hover:scale-[1.02] transition"
                    >
                        {showSnapshotPanel ? 'بستن اسنپ‌شات' : 'اسنپ‌شات Locate'}
                    </button>
                </div>
                {showImportPanel && (
                ...
                {showSnapshotPanel && (
                    <div className="absolute top-14 left-4 right-4 z-[550] max-h-[80vh] overflow-auto">
                        <SnapshotLocate onResult={handleSnapshotResult} />
                    </div>
                )}
                {isLoadingTowers && (
                ...
                <MapView 
                    pins={pins}
                    towers={towers}
                    snapshotTowers={snapshotResult?.anchors ?? []}
                    circles={circles}
                    rulerState={rulerState}
                    selectedPinId={selectedPinId}
                    phoneLocation={snapshotResult?.phone_location ?? null}
                    computedLocation={
                        snapshotResult
                            ? {
                                  lat: snapshotResult.computed.location.lat,
                                  lng: snapshotResult.computed.location.lon,
                                  radius: snapshotResult.computed.radius,
                              }
                            : null
                    }
